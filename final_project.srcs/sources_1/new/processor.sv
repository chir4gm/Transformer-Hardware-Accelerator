...
module matrix_multiply #(
    parameter DATA_WIDTH = 16,
    parameter MATRIX_DIM = 4,    // For a 4x4 matrix
    parameter FIXED_POINT = 8
)(
    input  logic clk,
    input  logic rst_n,
    input  logic start,
    input  logic [DATA_WIDTH-1:0] matrix_a [MATRIX_DIM][MATRIX_DIM],  // Input matrix A
    input  logic [DATA_WIDTH-1:0] matrix_b [MATRIX_DIM][MATRIX_DIM],  // Input matrix B
    output logic [DATA_WIDTH-1:0] matrix_out [MATRIX_DIM][MATRIX_DIM], // Output matrix
    output logic all_done
);

    logic proc_done [MATRIX_DIM][MATRIX_DIM];
    logic [DATA_WIDTH-1:0] proc_out [MATRIX_DIM][MATRIX_DIM][MATRIX_DIM];

    // Generate grid of processors
    genvar i, j;
    generate
        for (i = 0; i < MATRIX_DIM; i++) begin : row_gen
            for (j = 0; j < MATRIX_DIM; j++) begin : col_gen
                logic [DATA_WIDTH-1:0] row_data [MATRIX_DIM];
                logic [DATA_WIDTH-1:0] col_data [MATRIX_DIM];

                // Static assignment of row and column data
                assign row_data = matrix_a[i];    // Assign entire row i
                for (genvar k = 0; k < MATRIX_DIM; k++) begin : col_assign
                    assign col_data[k] = matrix_b[k][j];    // Assign entire column j
                end
                ...
                processor #(
                    ...
                    .rst_n(rst_n),
                    .start(start),
                    .proc_mode(2'b11),  // MAC mode
                    .data_in_a(row_data),
                    .data_in_b(col_data),
                    .data_out(proc_out[i][j]),  // Direct connection to output matrix
                    .done(proc_done[i][j])
                );
                assign matrix_out[i][j] = proc_out[i][j][MATRIX_DIM - 1];
            end
        end
    endgenerate
    ...
    always_comb begin
        ...
        for (int r = 0; r < MATRIX_DIM; r++) begin
            for (int c = 0; c < MATRIX_DIM; c++) begin
                all_done = all_done && proc_done[r][c];
            end
        end
    end
